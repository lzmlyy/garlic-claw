import type { ChatMessagePart, ChatMessageStatus, JsonValue, PluginSubagentToolCall, PluginSubagentToolResult, SSEEvent } from '@garlic-claw/shared';
...
export type ConversationTaskEvent = Extract<SSEEvent, { type: 'finish' | 'message-patch' | 'status' | 'text-delta' | 'tool-call' | 'tool-result'; }>;
...
export class ConversationTaskService {
  ...
  startTask(input: StartConversationTaskInput): void {
    ...
    const task: ConversationTaskHandle = { abortController: new AbortController(), completion: new Promise<void>((resolve) => { resolveCompletion = resolve; }), listeners: new Set() };
    this.tasks.set(input.assistantMessageId, task);
    setTimeout(() => { void this.runTask(task, input).finally(() => { this.tasks.delete(input.assistantMessageId); resolveCompletion(); }); }, 0);
  }
  ...
  private async runTask(task: ConversationTaskHandle, input: StartConversationTaskInput): Promise<void> {
    ...
    try {
      ...
      await this.finish(task, resolvedInput, state, 'error', error instanceof Error ? error.message : 'Conversation generation failed');
    }
  }
  ...
  private async complete(
    ...
    if (patched && hasPatchedResult(completed, patched)) {
      await this.runtimeHostConversationMessageService.writeMessage(input.conversationId, input.assistantMessageId, { content: patched.content, model: patched.modelId, parts: patched.parts, provider: patched.providerId, status: 'completed', toolCalls: patched.toolCalls, toolResults: patched.toolResults });
      this.emit(task, { content: patched.content, messageId: input.assistantMessageId, ...(patched.parts.length > 0 ? { parts: patched.parts } : {}), type: 'message-patch' });
    }
  ...
  private async finish(task: ConversationTaskHandle, input: Pick<StartConversationTaskInput, 'assistantMessageId' | 'conversationId' | 'modelId' | 'providerId'>, state: { content: string; toolCalls: ConversationTaskToolCall[]; toolResults: ConversationTaskToolResult[] }, status: 'error' | 'stopped', error?: string): Promise<void> {
  ...
  private async persist(input: Pick<StartConversationTaskInput, 'assistantMessageId' | 'conversationId' | 'modelId' | 'providerId'>, state: { content: string; toolCalls: ConversationTaskToolCall[]; toolResults: ConversationTaskToolResult[] }, status: ChatMessageStatus, error: string | null): Promise<void> { await this.runtimeHostConversationMessageService.writeMessage(input.conversationId, input.assistantMessageId, { content: state.content, error, model: input.modelId, parts: toAssistantParts(state.content), provider: input.providerId, status, toolCalls: state.toolCalls, toolResults: state.toolResults }); }
...
function consumePart(state: { content: string; toolCalls: ConversationTaskToolCall[]; toolResults: ConversationTaskToolResult[] }, messageId: string, rawPart: unknown): ConversationTaskEvent | null {
...
function buildResult(input: Pick<StartConversationTaskInput, 'assistantMessageId' | 'conversationId' | 'modelId' | 'providerId'>, state: { content: string; toolCalls: ConversationTaskToolCall[]; toolResults: ConversationTaskToolResult[] }): CompletedConversationTaskResult { return { assistantMessageId: input.assistantMessageId, content: state.content.trim(), conversationId: input.conversationId, modelId: input.modelId, parts: toAssistantParts(state.content), providerId: input.providerId, toolCalls: state.toolCalls.map((entry) => ({ ...entry })), toolResults: state.toolResults.map((entry) => ({ ...entry })) }; }
