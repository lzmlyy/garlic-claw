...
export function useToolManagement() {
  ...
  async function setSourceEnabled(source: ToolSourceInfo, enabled: boolean) {
    ...
    notice.value = null
    try {
      await api.updateToolSourceEnabled(source.kind, source.id, enabled)
  ...
  async function setToolEnabled(tool: ToolInfo, enabled: boolean) {
    ...
    notice.value = null
    try {
      await api.updateToolEnabled(tool.toolId, enabled)
  ...
  async function runSourceAction(source: ToolSourceInfo, action: PluginActionName) {
    ...
    notice.value = null
    try {
      const result = await api.runToolSourceAction(source.kind, source.id, action)
